/* AppUpload — client-side pipeline.
   The server's lib/ (extract.js, pricing.js, formMaps.js, anchors.js, fillForm.js,
   menu.js, merchants.js, apiError.js) ported to run entirely in the browser:
   extraction calls the Anthropic API directly with the key saved in Settings,
   PDFs are composited with pdf-lib, and the Clover workbook is filled with
   ExcelJS — no backend required. Documents stay in memory. */
"use strict";
(function () {
...
function friendlyApiError(e) {
  ...
  switch (status) {
    case 401:
      message = "Authentication failed — the saved Anthropic API key is missing or invalid. Open Settings and check it's a real key from console.anthropic.com (starts with sk-ant-), copied in full with no extra spaces.";
      break;
    case 403:
      message = "The API key is valid but not permitted for this request. Check the key's workspace/permissions.";
      break;
    case 429:
      message = "Rate limited or out of credits. Check your Anthropic plan, usage limits, and billing.";
      break;
    case 529:
      message = "Anthropic's servers are overloaded right now — try again in a moment.";
      break;
    default:
...
const DEFAULT_MODEL = (window.AU_CONFIG && window.AU_CONFIG.model) || "claude-sonnet-4-6";
...
const EXTRACTION_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      appType: {
        type: "string",
        enum: ["citizens", "merrick", "fd_north", "pbt", "unknown"],
        description:
          "Which application template this is. 'citizens' and 'fd_north' share the same Priority 'Merchant Processing Application and Agreement' 3-page layout (numbered sections 1-10) — tell them apart by the member bank in the footer/confirmation page: 'citizens' mentions Citizens Bank, N.A., Providence RI (form code 59495v1 / PPS2301); 'fd_north' mentions Synovus Bank, Columbus GA (form code PRTH0125WP). 'merrick' = 'Merchant Application and Agreement' mentioning Merrick Bank / Woodbury NY. 'pbt' = the 6-page Priority 'Merchant Processing Application and Agreement' (Page 1 of 6, form code PPS0622v3) with the PB&T Bank logo, mentioning The Pueblo Bank and Trust Company / PB&T Bank, Pueblo CO. 'unknown' if it cannot be determined.",
      },
      ...
      fees: {
        ...
        properties: {
          ...
          electronicAvs: { type: "string", description: "Electronic AVS fee — Merrick only (preprinted $0.05 on Citizens/FD North; ignore there)" },
          voiceAuth: { type: "string", description: "Voice Authorization fee — Merrick only (preprinted on Citizens/FD North; ignore there)" },
          voiceAvs: { type: "string", description: "Voice AVS fee — Merrick only (preprinted on Citizens/FD North; ignore there)" },
...
- one or more pages of a handwritten merchant application (the Citizens/Priority form, the FD North/Synovus form — same Priority layout, Synovus Bank in the footer — the Merrick form, or the 6-page PB&T Bank/Priority form from Pueblo CO),
- a voided check,
...
6. FEES: read the "Service Acceptance and Fee Schedule" and "Authorization, Monthly & Miscellaneous Fees"
   sections and fill every fee that has a handwritten amount. On the Citizens/FD North forms, ignore preprinted
   amounts (e.g. Electronic AVS $0.05, Batch Fee $0.20, Voice Auth $1.00) unless the agent wrote a different
   value; on the Merrick form those cells are blank, so capture handwritten Electronic AVS / Voice Authorization /
   Voice AVS / Batch / Return Transaction amounts when present.
...
async function anthropicToolCall({ apiKey, model, system, tool, content }) {
  let res, data;
  try {
  ...
  if (!res.ok) {
    const base = new Error(data?.error?.message || `Request failed (${res.status}).`);
    base.status = res.status;
    throw friendlyApiError(base);
  }
...
async function extractFromImages(images, opts = {}) {
  ...
  if (!apiKey) {
    const err = new Error("No Anthropic API key saved. Open Settings and add one, or turn on demo mode.");
    err.statusCode = 503;
    throw err;
  }
  ...
  const input = await anthropicToolCall({ apiKey, model, system: SYSTEM_PROMPT, tool: EXTRACTION_TOOL, content });
...
function normalizeRecord(raw = {}) {
  ...
  const out = {
    appType: ["citizens", "merrick", "fd_north", "pbt"].includes(r.appType) ? r.appType : "unknown",
    ...
    po: { ...emptyPo(), ...(r.po || {}) },
    bankChange: { ...emptyBankChange(), ...(r.bankChange || {}) },
    crf: { ...emptyCrf(), ...(r.crf || {}) },
    cbd: { ...emptyCbd(), ...(r.cbd || {}) },
    giftCard: { ...emptyGiftCard(), ...(r.giftCard || {}) },
  ...
  out.business.dnb = str(out.business.dnb);
  ...
  const bcNum = ["merchantId", "fundRouting", "fundAccount", "billRouting", "billAccount"];
  bcNum.forEach((k) => { out.bankChange[k] = str(out.bankChange[k]); });
  out.crf.merchantId = str(out.crf.merchantId);
  out.crf.ebtFns = str(out.crf.ebtFns);
  const gcArr = (v, n, empty) => {
    const list = Array.isArray(v) ? v : v && typeof v === "object" ? Object.keys(v).sort((a, b) => a - b).map((k) => v[k]) : [];
    return Array.from({ length: n }, (_, i) => {
      const row = { ...empty() };
      for (const k of Object.keys(row)) row[k] = str((list[i] || {})[k]);
      return row;
    });
  };
  out.giftCard.eq = gcArr(out.giftCard.eq, 3, () => ({ qty: "", terminalType: "", bizType: "", model: "", serial: "" }));
  out.giftCard.locations = gcArr(out.giftCard.locations, 2, emptyGiftCardLoc);
  ["numLocations", "merchantProcessingNum", "giftCardMerchantNum", "omahaMerchantNum", "mcc",
   "affMerchantId", "affAltId", "affExistingNum", "affPromoNumber"].forEach((k) => { out.giftCard[k] = str(out.giftCard[k]); });
  out.cbd.stateHempLicense = str(out.cbd.stateHempLicense);
  // The review form writes products via numeric paths, which arrive as an
  // object keyed "0".."9" — accept either shape; keep 10 editable rows (20 max on the form).
  const prodsIn = out.cbd.products;
  const prodList = Array.isArray(prodsIn)
    ? prodsIn
    : prodsIn && typeof prodsIn === "object"
      ? Object.keys(prodsIn).sort((a, b) => a - b).map((k) => prodsIn[k])
      : [];
  out.cbd.products = Array.from({ length: Math.max(10, Math.min(20, prodList.length)) }, (_, i) => ({
    name: str(prodList[i]?.name), distributor: str(prodList[i]?.distributor), state: str(prodList[i]?.state),
  }));
  return out;
}
...
// Bank Account Change Request form (manually filled by the rep — not extracted).
const emptyBankChange = () => ({
  merchantId: "",
  fundBankName: "", fundRouting: "", fundAccount: "",
  billBankName: "", billRouting: "", billAccount: "",
  addrMatchesFile: false, addrChangeLegal: false, addrChangeDba: false,
  settleChargebacksToFunding: false,
  docVoidedCheck: false, docBankLetter: false, docBankStatement: false,
});

// Change Request Form (CRF) — a standalone one-page document; filled manually.
const emptyCrf = () => ({
  merchantId: "", ownerName: "", dba: "", legalName: "",
  dbaName: "", legalAddress: "", dbaAddress: "", emailAddress: "", dbaPhone: "", dbaFax: "", website: "",
  amexOptBluePlan: "", amexOptBlueRate: "", amexDirectSe: "", addDiscover: false,
  pinDebitDiscount: false, pinDebitRate: "", ebtFns: "", ebtFee: "", addCashBenefits: false,
  myMerchantBenefits: false, myMerchantBenefitsRate: "", vmdDiscount: false, vmdNewRate: "",
  checkCardDiscount: false, checkCardRate: "", other1: "", other1Rate: "", other2: "", other2Rate: "",
  notes: "",
});
// Clover/Omaha Gift Card setup packet — standalone document, filled manually.
const emptyGiftCardLoc = () => ({ dba: "", street: "", city: "", state: "", zip: "", phone: "", fax: "", contact: "", positionTitle: "", email: "", processingNum: "" });
const emptyGiftCard = () => ({
  appKind: "", tieTo: "", resubmission: false, additionalInfo: false, numLocations: "", // appKind: 'new' | 'outlet' | 'entitle'
  eq: [], // up to 3 rows: { qty, terminalType, bizType, model, serial }
  merchantProcessingNum: "", giftCardMerchantNum: "", chain: "", omahaMerchantNum: "", mcc: "",
  setUpFee: "", addlLocFee: "", activationFee: "", redemptionFee: "", reloadFee: "", voidFee: "", balanceFee: "", otherFee: "", monthlyMinFee: "", monthlyFee: "",
  locations: [], // up to 2 additional-location blocks
  affBank: "", affMerchantId: "", affAltId: "", affExistingNum: "", affPromoNumber: "",
});
// Hemp/CBD disclosure + CBD amendment — standalone documents, filled manually.
const emptyCbd = () => ({
  agreementDate: "", stateHempLicense: "",
  growsHemp: "", manufacturesHemp: "", advertisesHemp: "", // 'yes' | 'no' | ''
  products: [], // schedule rows: { name, distributor, state }
});
const emptyPo = () => ({
...
const MODELS = MATRIX.map(([model, price, rental, category]) => ({ model, price, rental, category }));
...
function priceFor(modelStr) {
  ...
  // Short keys (Z9, P8, A30…) are only trusted when they appear as their own word
  // in the original text; bare containment would false-match custom names like
  // "XYZ-9000" (contains "z9") and put a wrong price on the purchase order.
  const words = new Set(String(modelStr).toLowerCase().split(/[^a-z0-9]+/).filter(Boolean));
  let best = null;
  for (const e of ENTRIES) for (const c of cands) {
    if (!c.includes(e.key)) continue;
    if (e.key.length < 5 && !words.has(e.key)) continue;
    if (!best || e.key.length > best.key.length) best = e;
  }
  if (best) return best;
  // 3. a model key starts with the query — the closest (shortest) such model wins,
  // but if several models tie at that shortest length the query is ambiguous
  // (e.g. "A920" -> PRO vs MAX, "VP" -> VP100/VP350/...), so decline rather than
  // guess a price the sales rep didn't intend.
  let bestLen = Infinity, matches = [];
  for (const e of ENTRIES) for (const c of cands) if (e.key.startsWith(c)) {
    if (e.key.length < bestLen) { bestLen = e.key.length; matches = [e]; }
    else if (e.key.length === bestLen && !matches.includes(e)) matches.push(e);
  }
  return matches.length === 1 ? matches[0] : null;
}

/** True if the model string refers to a Clover point-of-sale device. */
function isClover(modelStr) {
...
async function fetchOk(url) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Missing template asset "${name}" — this form's page assets haven't been installed on this deployment yet.`);
  }
  return res;
}
async function loadAnchorsAsync(form) {
  if (anchorCache.has(form)) return anchorCache.get(form);
  const data = await (await fetchOk(`${ASSET_BASE}/anchors/${form}.json`)).json();
  anchorCache.set(form, data);
  return data;
}
function loadAnchors(form) {
  if (!anchorCache.has(form)) throw new Error(`Anchors for "${form}" not preloaded.`);
  return anchorCache.get(form);
}
async function renderedPageBytes(form, page) {
  const key = `${form}-p${page}`;
  if (pageCache.has(key)) return pageCache.get(key);
  const buf = new Uint8Array(await (await fetchOk(`${ASSET_BASE}/rendered/${key}.png`)).arrayBuffer());
  pageCache.set(key, buf);
  return buf;
}
async function preloadForm(form) {
  const anchors = await loadAnchorsAsync(form);
  for (let i = 1; i <= anchors.pages.length; i++) await renderedPageBytes(form, i);
  return anchors;
}
const anchorNorm = (s) => s.replace(/\s+/g, " ").trim().toLowerCase();
function findLabel(anchors, q) {
  const page = anchors.pages[(q.page || 1) - 1];
  if (!page) return null;
  const needle = anchorNorm(q.text);
  const matches = page.items.filter((it) => {
    const s = anchorNorm(it.str);
...
const FORM_MAPS = { citizens, merrick, coversheet, purchase_order, clover_addendum, bank_change, fd_north, pbt, crf, hemp_cbd, cbd_amendment, gift_card };
 ...
 */
async function fillForm(form, record) {
  const anchors = await preloadForm(form);
  ...
  for (let i = 0; i < anchors.pages.length; i++) {
    ...
    const png = await pdf.embedPng(await renderedPageBytes(form, i + 1));
  ...
  if (sigData && map.sign && map.sign.length && /^data:image\/png;base64,/.test(String(sigData))) {
    try {
      ...
      const png = await pdf.embedPng(b64ToBytes(b64));
 ...
 */
function computePlacements(form, record, measure) {
  const map = FORM_MAPS[form];
 ...
 */
function resolveForm(record, formOverride) {
...
function hasCloverEquipment(record) {
 ...
 */
function prepareRecord(record, opts = {}) {
...
async function buildPacket(record, opts = {}) {
...
const MENU_TEMPLATE_URL = `${ASSET_BASE}/templates/clover_menu_template.xlsx`;
...
const MENU_SYSTEM_PROMPT = `You are an expert at reading restaurant menus from photos and converting them into a \
...
async function extractMenu(images, opts = {}) {
  ...
  const input = await anthropicToolCall({ apiKey, model, system: MENU_SYSTEM_PROMPT, tool: MENU_TOOL, content });
 ...
 */
async function buildCloverWorkbook(menu) {
  const wb = new window.ExcelJS.Workbook();
  await wb.xlsx.load(await (await fetchOk(MENU_TEMPLATE_URL)).arrayBuffer());
...
/* ---------------- merchant directory (assets/merchants.xlsx, optional) ---------------- */
let merchantsCache = null;
async function listMerchants() {
  if (merchantsCache) return merchantsCache;
  try {
    const res = await fetch(`${ASSET_BASE}/merchants.xlsx`);
    if (!res.ok) return (merchantsCache = []);
    const wb = new window.ExcelJS.Workbook();
    await wb.xlsx.load(await res.arrayBuffer());
    const ws = wb.worksheets[0];
    if (!ws) return (merchantsCache = []);
    const cellText = (cell) => {
      let v = cell.value;
    ...
    ws.getRow(1).eachCell({ includeEmpty: true }, (cell, c) => {
      const k = cellText(cell).toLowerCase();
      if (k && !(k in header)) header[k] = c;
    });
    const col = (names) => names.map((n) => header[n]).find((c) => c);
    const cDba = col(["dba", "dba name", "merchant", "merchant name", "name"]);
    const cMid = col(["mid", "merchant id", "merchant #", "merchant number"]);
    const rows = [];
    ws.eachRow((row, rn) => {
      if (rn === 1) return;
      const dba = cDba ? cellText(row.getCell(cDba)) : "";
      const mid = cMid ? cellText(row.getCell(cMid)) : "";
      if (dba) rows.push({ dba, mid });
    });
    return (merchantsCache = rows);
  } catch {
